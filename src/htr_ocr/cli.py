...
from htr_ocr.train.hybrid_fusion_infer import (
    infer_one as hybrid_fusion_infer_one,
    load_checkpoint as hybrid_fusion_load_checkpoint,
)
from htr_ocr.train.hybrid_fusion_trainer import (
    evaluate as hybrid_fusion_evaluate,
    make_dataloader as hybrid_fusion_make_dataloader,
    train_hybrid_fusion_ctc,
)
...
class HTRCLI:
    ...
    def train_hybrid_fusion_ctc(self, *overrides: str) -> None:
        cfg = load_cfg("train_hybrid_fusion_ctc", overrides=list(overrides))

        with mlflow_run("train_hybrid_fusion_ctc", cfg):
            result = train_hybrid_fusion_ctc(cfg)

            device = torch.device(cfg.train.device if torch.cuda.is_available() else "cpu")
            model, tok = hybrid_fusion_load_checkpoint(result.best_checkpoint, device)
            test_dl = hybrid_fusion_make_dataloader(cfg, "test")
            metrics = hybrid_fusion_evaluate(model, test_dl, tok, device, decode_cfg=cfg.decode)
    ...
    def eval_hybrid_fusion_ctc(self, *overrides: str) -> None:
        cfg = load_cfg("eval_hybrid_fusion_ctc", overrides=list(overrides))
        ...
        with mlflow_run("eval_hybrid_fusion_ctc", cfg, extra_tags={"split": split_name}):
            device = torch.device(cfg.eval.device if torch.cuda.is_available() else "cpu")
            model, tok = hybrid_fusion_load_checkpoint(ckpt_path, device)
            dl = hybrid_fusion_make_dataloader(cfg, split_name)
            metrics = hybrid_fusion_evaluate(model, dl, tok, device, decode_cfg=cfg.decode)
    ...
    def infer_hybrid_fusion_ctc(self, *overrides: str) -> None:
        cfg = load_cfg("infer_hybrid_fusion_ctc", overrides=list(overrides))
